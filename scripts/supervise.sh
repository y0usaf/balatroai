...
# This restarts it from the newest checkpoint, with a cap and a backoff so a
# genuinely broken configuration fails loudly instead of thrashing.
#
#   setsid nohup nix develop -c bash scripts/supervise.sh runs/pointer_v7 \
#       > runs/pointer_v7/supervisor.log 2>&1 < /dev/null &
#
# Everything after the run directory is passed through to the trainer.
set -uo pipefail
cd "$(dirname "$0")/.."

RUN_DIR=${1:?usage: supervise.sh RUN_DIR [trainer args...]}
shift
